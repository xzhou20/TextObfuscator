...
def main():
    ...
    # hyperparameter for PrivacyLoss
    config.eps = args.eps
    config.w_cluster_away = args.w_cluster_away
    config.w_cluster_close = args.w_cluster_close
    
    config.cluster_num = len(label_list)
    
    knn_topk=10

    from models.modeling_roberta_privacy import RobertaForTokenClassification
    ...
    def tokenize_and_align_labels(examples):
        ...
        for i, label in enumerate(examples[label_column_name]):
            ...
            for idx, word_idx in enumerate(word_ids):
                ...
                if word_idx is None:
                    ...
                    cluster_id.append(-100)
                # We set the label for the first token of each word.
                elif word_idx != previous_word_idx:
                    label_ids.append(label_to_id[label[word_idx]])
                    cluster_id.append(label_to_id[label[word_idx]])
                ...
                # the label_all_tokens flag.
                    ...
                    if args.label_all_tokens:
                        ...
                        label_ids.append(-100)
                    if args.cluster_subword:
                        cluster_id.append(b_to_i_label[label_to_id[label[word_idx]]])
                    else:
                        cluster_id.append(-100)
        ...
        # special_tokens_mask 会使用1去padding，而此处我们使用-100作为padding，因此需要和attention_mask联合使用
        # tokenized_inputs['special_tokens_mask'] = cluster_ids
        # tokenized_inputs['cluster_ids'] = [[token2cluster[token] if token in token2cluster else 0 for token in sentence] for sentence in tokenized_inputs['input_ids']]
    ...
    import time
    from cluster_utils import generate_token_prototype
    previous_metric = 0
    tolerate = 10
    early_stop = tolerate
    privacy_target = {'conll2003':0.022, 'tner/ontonotes5':0.03}
    best_performance = 0
    
    
    print('init prototypes......')
    config.w_noise = 0
    config.target_break = 1
    start_time = time.time()
    cluster_center = generate_token_prototype(model, train_dataloader, args.target_layer, )
    ...
    print('init prototypes......done!cost {} seconds'.format(time.time()-start_time))
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.resume_from_checkpoint and epoch == starting_epoch:
                if resume_step is not None and step < resume_step:
                    completed_steps += 1
                    continue
            batch['cluster_ids'] = batch['labels']
            outputs = model(**batch)
        ...
        if epoch >= 0:
            ...
            if eval_metric['knn_top{}'.format(knn_topk)] <= privacy_target[args.dataset_name]:
                if eval_metric['f1'] > best_performance:
                    best_performance = eval_metric['f1']
